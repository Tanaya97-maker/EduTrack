    ...
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-6 duration-500">
      ...
      {showAddForm && (
        <div className="bg-white border-2 border-indigo-100 rounded-[2.5rem] p-10 space-y-8 shadow-2xl shadow-indigo-100/50 animate-in zoom-in-95 duration-200 relative overflow-hidden">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
            <div className="space-y-2">
              ...
              >
                ...
                {subjects
                  ...
                  .map(s => (
                    <option key={s.subject_id} value={s.subject_id}>{s.subject_code}: {s.subject_name} ({s.semester})</option>
                  ))}
              </select>
            </div>
            <div className="space-y-2">
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-300 uppercase tracking-widest ml-1">Hall</label>
          ...
          <div className="flex justify-end gap-4 border-t border-slate-50 pt-8 mt-2">
            <button onClick={() => setShowAddForm(false)} className="px-8 py-4 text-slate-400 font-black text-sm hover:text-slate-600 transition-colors">Cancel</button>
            <button onClick={handleAdd} className="bg-indigo-600 text-white font-black px-12 py-4 rounded-2xl shadow-xl shadow-indigo-100 hover:bg-indigo-700 active:scale-95 transition-all outline-none">Add Slot</button>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-6">
        {DAYS_OF_WEEK.map((day, dIdx) => {
          const daySlots = filteredTimetable.filter(t => Number(t.day_of_week) === (dIdx + 1));
          return (
              ...
              <div className="flex-1 space-y-4 min-h-[400px]">
                {daySlots.length > 0 ? (
                  daySlots.sort((a, b) => a.start_time.localeCompare(b.start_time)).map(slot => {
                    const sub = subjects.find(s => Number(s.subject_id) === Number(slot.subject_id));
                    return (
                      <div key={slot.timetable_id} className="bg-white p-5 rounded-[1.75rem] border border-slate-100 shadow-sm hover:shadow-xl hover:shadow-indigo-500/10 hover:border-indigo-100 transition-all group relative overflow-hidden">
                        ...
                        <div className="bg-indigo-50/50 text-indigo-600 text-[9px] font-black px-2.5 py-1 rounded-lg inline-block uppercase tracking-widest mb-3 border border-indigo-100/30">
                          {sub?.subject_code || 'MOD001'} {sub?.semester ? `• ${sub.semester.toUpperCase()}` : ''}
                        </div>
                  ...
                  <div className="h-40 flex flex-col items-center justify-center border-2 border-dashed border-slate-100 rounded-[2rem] text-center p-6 bg-slate-50/30">
                    ...
                    <div className="text-[9px] text-slate-300 font-bold uppercase tracking-[0.2em]">No Schedule Found</div>
                  </div>
                )}
              </div>
            </div>
