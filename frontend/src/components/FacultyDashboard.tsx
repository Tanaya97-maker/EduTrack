...
const FacultyDashboard: React.FC<Props> = ({
  ...
  const facultySubjects = subjects.filter(s => Number(s.faculty_id) === Number(faculty.faculty_id));
  const facultySlots = timetable.filter(slot => facultySubjects.some(s => Number(s.subject_id) === Number(slot.subject_id)));
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {facultySubjects.map(subject => (
              <div
                ...
                className={`cursor-pointer p-6 rounded-xl border-2 transition-all ${selectedSubject?.subject_id === subject.subject_id
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {enrolledInSelected.map(student => {
                      const record = attendance.find(
                        r => Number(r.stud_id) === Number(student.stud_id) &&
                          Number(r.subject_id) === Number(selectedSubject.subject_id) &&
                      ...
                      return (
                        <tr key={student.stud_id} className="hover:bg-slate-50 transition-colors">
                          ...
                          <td className="px-6 py-4 text-center">
                            {record ? (
                              <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${record.status === AttendanceStatus.PRESENT
                          ...
                          <td className="px-6 py-4 text-right">
                            <button
                              ...
                              className={`inline-flex items-center px-4 py-2 rounded-lg text-sm font-semibold transition-all shadow-sm ${record?.status === AttendanceStatus.PRESENT
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {DAYS_OF_WEEK.map((day, idx) => {
              const daySlots = facultySlots.filter(s => Number(s.day_of_week) === (idx + 1));
              return (
                <div key={day} className="space-y-4">
                  ...
                  {daySlots.length > 0 ? (
                    daySlots.sort((a, b) => a.start_time.localeCompare(b.start_time)).map(slot => {
                      const subject = subjects.find(s => Number(s.subject_id) === Number(slot.subject_id));
                      return (
